...
| 11 | **GDPR deletion** endpoint tested and verified | **Not done** | No documented `DELETE /me/account` (or equivalent) user-data erasure flow in FastAPI; deletion appears only in **tests** fixtures. Production GDPR compliance needs explicit API + Clerk user deletion policy + tests. |
