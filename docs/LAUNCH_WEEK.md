...
1. **Celery Beat + worker** (recommended baseline): same env as the API, Redis broker, worker listening on `default,scrape,score,draft,notify`, Beat process running. The default beat schedule refreshes RapidAPI JSearch and Active Jobs DB daily.
2. **Cron HTTP hook**: set **`DOUBOW_CRON_INGEST_SECRET`** on the API (long random string). **`POST /jobs/cron/queue-ingest`** with header **`X-Doubow-Cron-Secret`** queues RapidAPI JSearch + Active Jobs DB. Returns **404** if the secret is unset (endpoint hidden). To wipe ingested rows before a fresh pull, **`POST /jobs/cron/clear-catalog?mode=providers`** (same header; optional **`mode=all`** deletes every job including manual).
...
4. **Bootstrap on API startup** (optional): **`DOUBOW_BOOTSTRAP_INGEST_ON_STARTUP=true`** queues RapidAPI JSearch + Active Jobs DB ingest plus Glassdoor company enrichment **once** after deploy (Redis NX lock so multiple replicas do not duplicate). Still requires a **worker** to drain the queue.
