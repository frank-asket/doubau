...
4. **Bootstrap on API startup** (optional): **`DOUBOW_BOOTSTRAP_INGEST_ON_STARTUP=true`** queues JSearch + Remote OK ingest **once** after deploy (Redis NX lock so multiple replicas do not duplicate). Still requires a **worker** to drain the queue.
