...
import { ChromeIconButton, ChromeIconLink } from "@/components/ui/chrome-motion";
import { AppIcon } from "@/components/ui/app-icon";

import { useWorkspaceNotificationDot, WorkspaceCommandPalette } from "./WorkspaceCommandPalette";
...
export function AppTopbar() {
  ...
  const showNotifDot = useWorkspaceNotificationDot();
  ...
  return (
    <>
      <WorkspaceCommandPalette open={paletteOpen} onOpenChange={setPaletteOpen} />
      <header className="sticky top-0 z-20 shrink-0 bg-[linear-gradient(180deg,rgba(244,247,242,0.98),rgba(244,247,242,0.78))] px-2 pt-3 backdrop-blur md:px-3">
        <div className="doubow-orb flex h-[var(--app-topbar-h)] items-center justify-between gap-4 rounded-[32px] border border-[color-mix(in_srgb,white_72%,var(--app-border))] bg-[rgba(255,255,255,0.88)] px-5 shadow-[var(--app-shadow-1)] backdrop-blur-xl">
          ...
          <div className="flex shrink-0 items-center gap-2 md:gap-3">
            ...
            <button
              type="button"
              className="relative hidden h-11 w-[min(100%,320px)] items-center gap-3 rounded-full border border-[var(--app-border)] bg-white/88 px-4 text-left text-[14px] text-[var(--app-text-tertiary)] shadow-[var(--app-shadow-0)] lg:inline-flex"
              onClick={() => setPaletteOpen(true)}
            >
              <AppIcon name="search" className="size-5 shrink-0 text-[var(--app-text-secondary)]" aria-hidden />
              <span className="min-w-0 flex-1 truncate">Search workspace…</span>
              <kbd className="shrink-0 rounded-full bg-[var(--app-bg-muted)] px-2 py-1 text-[11px] font-semibold text-[var(--app-text-secondary)]">
                ⌘K
              </kbd>
            </button>
            <ChromeIconLink href="/app/notifications" className="relative inline-flex" aria-label="Notifications">
              <AppIcon name="bell" className="size-5" />
              {showNotifDot ? (
                <span
                  className="absolute right-3 top-3 inline-flex size-2 rounded-full bg-[var(--app-accent)] shadow-[0_0_0_6px_rgba(32,209,125,0.12)]"
                  aria-hidden
                />
              ) : null}
            </ChromeIconLink>
